...
public class Main {
	public static void main(String[] args) {
		...
		input = input.substring(schemaEnd + 2); // entferne <schema>://

		// Verarbeite Nutzer und Kennwort gemeinsam
		int authEnd = input.indexOf("@");
		if (authEnd != -1) {
			String auth = input.substring(1, authEnd);
			...
			input = input.substring(authEnd); // entferne <user>:<pass>@
		}
		...
		if (input.length() > 0) {
			...
			if (pfadBegin != -1) {
				pfad = input.substring(pfadBegin);
